...
在练习中，我尝试通过子类重写父类的私有方法 `__calc_area` 来修改父类的私有属性 `__area`，结果发现打印始终为 `0.0`。

**原因分析**：

1. 父类的 `__area` 被改名为 `_Shape__area`。

2. 子类定义的 `__area` 被改名为 `_Circle__area`。
 
3. 子类调用的 `__calc_area` 实际上是子类自己的私有方法，它修改的是子类的坑位，父类的坑位始终没变。     

 **结论**：**私有方法无法被重写**。如果一个方法需要多态特性，必须使用公开或受保护（单下划线）的命名方式。
